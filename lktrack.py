...
# from https://github.com/jesolem/PCV/blob/master/examples/lktrack.py
import time
...
feature_params = dict(maxCorners=50,qualityLevel=0.01,minDistance=10)
...
class LKTracker(object):
    ...
    def detect_points(self):
        ...
        self.features = features # [features[0,:], features[-1,:]] #features
    ...
    def draw(self):
        ...
        for point in self.features:
            ...
            #cv2.circle(self.image,(int(point[0][1]),int(point[0][0])),3,(0,255,0),-1)
        cv2.imshow('LKtrack',self.image)
        cv2.waitKey()
        
    def write(self):
        # writes tracked image
        # draw points as green circles
        for point in self.features:
            #print point
            cv2.circle(self.image,(int(point[0][0]),int(point[0][1])),3,(0,255,0),-1)
        fileName = './trackedImages/' + 'imgT' + str(self.tn) +'.jpg'
        print fileName
